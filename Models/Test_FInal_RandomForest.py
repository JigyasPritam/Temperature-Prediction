...
data_new = data_new.dropna(subset=["datetime"])
...
X_train, X_validate, Y_train_max, Y_validate_max, Y_train_min, Y_validate_min = train_test_split(
    X_train, Y_train_max, Y_train_min, test_size=0.2, random_state=42
)

# Train Random Forest model for max temperature
rf_model_max = RandomForestRegressor(n_estimators=50, random_state=42)
rf_model_max.fit(X_train, Y_train_max)

# Train Random Forest model for min temperature
rf_model_min = RandomForestRegressor(n_estimators=50, random_state=42)
rf_model_min.fit(X_train, Y_train_min)

# Make predictions for max temperature
Y_pred_test_max = rf_model_max.predict(X_test)
Y_pred_train_max = rf_model_max.predict(X_train)

# Make predictions for min temperature
Y_pred_test_min = rf_model_min.predict(X_test)
Y_pred_train_min = rf_model_min.predict(X_train)
...
# Fixed Reference line (Actual = Predicted)
min_val = min(min(Y_test_max), min(Y_pred_test_max))
max_val = max(max(Y_test_max), max(Y_pred_test_max))
...
plt.title("Actual vs Predicted Max Temperatures (Random Forest)")
plt.legend()
...
# Fixed Reference line (Actual = Predicted)
min_val = min(min(Y_test_min), min(Y_pred_test_min))
max_val = max(max(Y_test_min), max(Y_pred_test_min))
plt.plot([min_val, max_val], [min_val, max_val], color="red", linestyle="dashed", label="Ideal Fit")

plt.xlabel("Actual Min Temperature")
plt.ylabel("Predicted Min Temperature")
plt.title("Actual vs Predicted Min Temperatures (Random Forest)")
plt.legend()
